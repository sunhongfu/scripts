...
if sum(strcmpi('tfi',bkg_rm))
    ...
    % nii = make_nii(QSM.*Mask,vox);
    % save_nii(nii,'TFI/TFI_lambda1000_full.nii');
    ...
    % QSM = TFI_L1('filename', 'RDF_brain.mat', 'lambda', 1000);
    % nii = make_nii(QSM.*Mask,vox);
    % save_nii(nii,'TFI/TFI_lambda1000_ero1.nii');
    % % QSM = TFI_L1('filename', 'RDF_brain.mat', 'lambda', 1500);
    % % nii = make_nii(QSM.*Mask,vox);
    ...
    % QSM = TFI_L1('filename', 'RDF_brain.mat', 'lambda', 1000);
    % nii = make_nii(QSM.*Mask,vox);
    % save_nii(nii,'TFI/TFI_lambda1000_ero2.nii');
    % % QSM = TFI_L1('filename', 'RDF_brain.mat', 'lambda', 1500);
    % % nii = make_nii(QSM.*Mask,vox);
    ...
    QSM = TFI_L1('filename', 'RDF_brain.mat', 'lambda', 1000);
    nii = make_nii(QSM.*Mask,vox);
    save_nii(nii,'TFI/TFI_lambda1000_ero3.nii');
    % QSM = TFI_L1('filename', 'RDF_brain.mat', 'lambda', 1500);
    % nii = make_nii(QSM.*Mask,vox);
...
if sum(strcmpi('lbv',bkg_rm))
   ...
   % MEDI
    ...
    save_nii(nii,['LBV/MEDI2000_LBV_smvrad3.nii']);
