...
for echo = 1:4
    nii = load_untouch_nii(['/Volumes/LaCie_Top/AHEAD/sample-data_highres/subject_inv-2_echo-' num2str(echo) '_part-mag.nii']);
    % mag(:,:,:,echo) = double(nii.img +32768);
    mag(:,:,:,echo) = double(nii.img);

    nii = load_untouch_nii(['/Volumes/LaCie_Top/AHEAD/sample-data_highres/subject_inv-2_echo-' num2str(echo) '_part-ph.nii']);
...
% for echo_num = 1:imsize(4)
%     unph_lap(:,:,:,echo_num) = unwrapLaplacian(ph_corr(:,:,:,echo_num), imsize(1:3), vox);
%     % unph_lap(:,:,:,echo_num) = lapunwrap(ph_corr(:,:,:,echo_num), Options).*mask;
%     % unph_lap(:,:,:,echo_num) = MRPhaseUnwrap(ph_corr(:,:,:,echo_num),'voxelsize',vox,'padsize',[12 12 12]).*mask;
% end
