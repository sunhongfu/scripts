...
if ($ARGC < 4) {
    ...
    print ("Usage: multi_echo_swi_unwrapped mag_file_base phase_file_base mask_file num_echoes output.mnc [first_echo_index] [weights]");
    die;
}
...
#     run_matlab("do_unwrap_swi('$mag', '$phase', '/tmp/swi_unwrap_echo_${i}_of_${num_echoes}.mnc', '/tmp/phase_unwrap_echo${i}_of_${num_echoes}.mnc', '$mask',  $windowSizeRatio, 1, 4, 1)");
#     $windowSizeRatio = $windowSizeRatio + 0.05;
    
    
# #    `minc_nuyl /tmp/swi_unwrap_echo_${i}_of_${num_echoes}.mnc /tmp/swi_unwrap_echo_1_of_${num_echoes}.mnc /tmp/swi_unwrap_echo_${i}_of_${num_echoes}_rescaled.mnc`;
    
#     #`VolumeRescale "/tmp/swi_unwrap_echo_${i}_of_${num_echoes}.mnc" "/tmp/swi_unwrap_echo_${i}_of_${num_echoes}_rescaled.mnc" 0 1000`;
# }
...
$cmd = "${cmd})/${sum}' ";
for ($i=$first_echo_index; $i<($first_echo_index+$num_echoes); $i++) {
    $cmd = "${cmd} swi_unwrap_echo_${i}_of_${num_echoes}.mnc ";
}
$cmd = "${cmd} ${output_file}";
