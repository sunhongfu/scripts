...
mkdir('superposition_2015_01_15');
cd('superposition_2015_01_15');
...
weights = abs(img_cmb)./max(abs(img_cmb(:))).*mask_resharp.*reliability;
...
sus_masked_resharp = tvdi(lfs_resharp, mask_resharp, voxelSize, tv_reg, weights, z_prjs, 20);
sus_masked_resharp = sus_masked_resharp.*mask_resharp;
nii = make_nii(sus_masked_resharp.*mask_resharp,voxelSize);
save_nii(nii,'sus_masked_resharp.nii');
