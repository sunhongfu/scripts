...
sus_masked_lbv = tvdi(lfs_lbv, mask_lbv, voxelSize,tv_reg,weights.^2,z_prjs); 
sus_masked_lbv = sus_masked_lbv.*mask_lbv;
nii = make_nii(sus_masked_lbv.*mask_lbv,voxelSize);
save_nii(nii,['sus_masked_lbv_' num2str(thr) '.nii']);
...
hemo_mask(sus_masked_lbv>=0.5) = 0;
...
save_nii(nii,['lfs_noHemo_lbv1_' num2str(thr) '.nii']);
...
save_nii(nii,['sus_noHemo_lbv_' num2str(thr) '.nii']);
...
save_nii(nii,['sus_super_lbv1_' num2str(thr) '.nii']);
...
save_nii(nii,['sus_super_fix_lbv1_' num2str(thr) '.nii']);
