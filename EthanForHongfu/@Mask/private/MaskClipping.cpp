/************************************************************************** 
                        ...
                        Mask Clipping Algorithm
 
                                    Created by M Ethan MacDonald
 
   This mex function has the utility of removing stray rays from the ray 
 casting brain extraction algorithm.
 ...
 It would be good to select >10. The output is the new mask.
 
   The algorithm will fill in points in the mask until there are no more voxels 
 with less empty adjacent voxels.
 
 Example Syntax:
 
    [ OutputMask ] = MaskClipping( OldMask, numAdjacentEmptyVoxels ) ;
...
public:
...
#ifdef DEBUG_MASK_CLIPPING
    printf( "DEBUG_MASK_CLIPPING: starting mask filling process\n" ) ;
    printf( "DEBUG_MASK_CLIPPING: \n"
...
#ifdef DEBUG_MASK_CLIPPING
    printf( "DEBUG_MASK_CLIPPING: finished initializing the mask\n" ) ;
#endif
        passCount = -1 ;
...
#ifdef DEBUG_MASK_CLIPPING
    printf( "DEBUG_MASK_CLIPPING: pass %i / %i, numPtsAddedThisPass = %i\n",
...
#ifdef DEBUG_MASK_CLIPPING
    printf( "DEBUG_MASK_CLIPPING: finishing mask filling process\n" ) ;
#endif
    ...
    void check_all_pts_in_mask( void ) {
        ...
        for( int xCount = 0 ; xCount < dims[0] ; xCount++ ) {
            ...
            if( OutputMask[(xCount)+dims[0]*(yCount)+dims[0]*dims[1]*(zCount)] == 1.0 ) {
            ...
            //printf( "getting here again\n" ) ; 
                ...
                if( sum_of_adjacent < threshold ) {
                    OutputMask[(xCount)+dims[0]*(yCount)+dims[0]*dims[1]*(zCount)] = 0.0 ;
                    numPtsAddedThisPass++ ;
                }
...
#ifdef DEBUG_MASK_CLIPPING
    printf( "DEBUG_MASK_CLIPPING: starting function\n" ) ;
#endif
...
#ifdef DEBUG_MASK_CLIPPING
    printf( "DEBUG_MASK_CLIPPING: read back inputs\n"
            "\t threshold = %i \n"
            "\t dimensions = %i %i %i \n",
            (int)Threshold[0], 
...
#ifdef DEBUG_MASK_CLIPPING    
    printf( "DEBUG_MASK_CLIPPING: number of points filled in = %i\n", 
