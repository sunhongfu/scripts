...
class yangDataSet(data.Dataset):
    def __init__(self, root, list_path):
        ...
        for name in self.img_ids:
            ...
            """
            label_file = self.root + ("/Label_k64/%s_Label_k.nii" % name)
            mask_file = self.root + ("/mask_img/%s_mask.nii" % name)

            Dipole_file = ("./Dipole_k64/Dipole_%s.nii" % str(1))
    ...
    def __getitem__(self, index):
        ...
        input_k[:,:,:,1] = torch.zeros(64,64,64)
