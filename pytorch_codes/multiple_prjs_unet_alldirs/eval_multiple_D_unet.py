...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        for orien in ['central_bigAngle']:
            ...
            multiple_prjs_unet_alldirs = Unet(2)
            multiple_prjs_unet_alldirs = nn.DataParallel(
                multiple_prjs_unet_alldirs)
            device = torch.device(
                "cuda:0" if torch.cuda.is_available() else "cpu")
            multiple_prjs_unet_alldirs.load_state_dict(torch.load(
                '/scratch/itee/uqhsun8/CommQSM/pytorch_codes/multiple_prjs_unet_alldirs/multiple_prjs_unet_alldirs.pth'))
            multiple_prjs_unet_alldirs.to(device)
            multiple_prjs_unet_alldirs.eval()
            print(multiple_prjs_unet_alldirs.state_dict)
            ################ Evaluation ##################
            image = image.to(device)
            prjs = prjs.to(device)
            pred = multiple_prjs_unet_alldirs(image, prjs)
            print(pred.size())
            pred = torch.squeeze(pred, 0)
            pred = torch.squeeze(pred, 0)
            print(get_parameter_number(multiple_prjs_unet_alldirs))
            pred = pred.to('cpu')
            pred = pred.numpy()

            name_msk = '/scratch/itee/uqhsun8/CommQSM/invivo/testing/multiple_prjs_unet_alldirs/renzo_' + \
                orien + '_multiple_prjs_unet_alldirs.nii'
            path = '/scratch/itee/uqhsun8/CommQSM/invivo/testing/multiple_prjs_unet_alldirs/renzo_' + \
                orien + '_multiple_prjs_unet_alldirs.mat'
