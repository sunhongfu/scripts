...
STEP_SIZE = nn.Parameter(torch.ones(1)*4)
...
class DataConsistency(nn.Module):
    ...
    def forward(self, chi, phi, D):
        ...
        chi = torch.cat([chi, torch.zeros(chi.shape).cuda()], 1)
        phi = torch.cat([phi, torch.zeros(phi.shape).cuda()], 1)
...
class Grad(nn.Module):
    ...
    def forward(self, chi, phi, D):
        x = self.dc(chi, phi, D)
        if str(x.device)[5] == '0':
            self.__stepSize = nn.Parameter(self.__stepSize.to(device=torch.device("cuda:0")))
        elif str(x.device)[5] == '1':
            self.__stepSize = nn.Parameter(self.__stepSize.to(device=torch.device("cuda:1")))
        x = chi - self.__stepSize * x
        return x
