...
def SaveNet(net, enSave=False):
    ...
    if enSave:
        torch.save(
            net, '/scratch/itee/uqhsun8/zhuang/unrolledQSM_WideResNet_torchParams/res.pth')
    else:
        torch.save(net.state_dict(),
                   '/scratch/itee/uqhsun8/zhuang/unrolledQSM_WideResNet_torchParams/res.pth')


def TrainNet(net, LR=LEARN_RATE, Batchsize=32, Epoches=100, useGPU=False, RESUME=True, path_checkpoint='./res.pth', save_folder='./checkpoints'):
    print('unrolledQSM')
    print('DataLoad')
        ...
        if torch.cuda.is_available():
            ...
            for epoch in range(1, Epoches + 1):
                ...
                for i, values in enumerate(trainloader):
                    ...
                    fields = fields.to(device,dtype=torch.float)
                    Dipoles = Dipoles.to(device,dtype=torch.float)
                ...
                if epoch % 10 == 0:
                    # save_checkpoints(net, optimizer,
                                     # scheduler, epoch+50, save_folder)
                    save_checkpoints(net, optimizer,
                                     scheduler, epoch+50, save_folder)
        else:
            pass
...
if __name__ == '__main__':
    ...
    # train network
    #TrainNet(net, LR=0.001, Batchsize=16, Epoches=50, useGPU=True)
    TrainNet(net, LR=0.001, Batchsize=16, Epoches=50, useGPU=True, RESUME=True,
