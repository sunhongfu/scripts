...
# traditional U-net.
# this programme is in the similar structure withe ResNet_yang.py
#################### Section 1 ###########################
from yangOctNetBlocks import *
################# End Section 1 ##########################
...
class OctNet(nn.Module):
    def __init__(self, EncodingDepth):
        ...
        self.InputOct = OctEncodingBlocks(
            1, initial_num_layers, alphax=1, alphay=0.5)
################### Encoding Layers #######################
        for encodingLayer in temp:
            if encodingLayer == 1:
                num_outputs = initial_num_layers * 2 ** (encodingLayer - 1)
                self.EncodeConvs.append(OctEncodingBlocks(
                    initial_num_layers, num_outputs))
        ...
        for decodingLayer in temp:
            if decodingLayer == EncodingDepth:
                ...
                self.DecodeConvs.append(
                    OctDecodingBlocks(num_inputs, num_inputs))
            else:
                num_inputs = initial_decode_num_ch // 2 ** (decodingLayer - 1)
                self.DecodeConvs.append(
                    OctDecodingBlocks(num_inputs, num_inputs // 2))
