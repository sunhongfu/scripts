...
fileID = fopen('/scratch/itee/uqhsun8/CommQSM/pytorch_codes/image_unet_stack_prjs_alldirs_150k/z_prjs_alldirs.txt','w');
...
for i = 1:15000
    ...
    for j = randi(length(z_prjs),[1,10])
        ...
        save_nii(nii,['/scratch/itee/uqhsun8/CommQSM/invivo/alldirs_chi/alldirs_chi_' num2str(i) '-' num2str(j) '.nii']);
        nii = make_nii(field, vox);
        save_nii(nii,['/scratch/itee/uqhsun8/CommQSM/invivo/alldirs_field/alldirs_field_' num2str(i) '-' num2str(j) '.nii']);
        % nii = make_nii(D, vox);
