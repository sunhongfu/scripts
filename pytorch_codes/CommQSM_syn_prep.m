...
for i = 1:999
    nii = load_nii(['/Volumes/LaCie/CommQSM/syn/QSM_label/' num2str(i), '-Phantom_NIFTI.nii']);
    img = single(nii.img);

    % generate D and Field
    vox = [1 1 1];
    for j = 1:size(z_prjs_all,1)
        [field, D] = forward_field_calc(img, vox, z_prjs_all(j,:));
        field_D_cat = cat(3,field,D);

        % save chi(img), field, D, field_D_cat as NIFTIs
        nii = make_nii(img, vox);
        save_nii(nii,['/Volumes/LaCie/CommQSM/syn/data_for_training/chi/chi_' num2str(i) '-' num2str(j) '.nii']);
        nii = make_nii(field, vox);
        save_nii(nii,['/Volumes/LaCie/CommQSM/syn/data_for_training/field/field_' num2str(i) '-' num2str(j) '.nii']);
        nii = make_nii(D, vox);
        save_nii(nii,['/Volumes/LaCie/CommQSM/syn/data_for_training/D/D_' num2str(i) '-' num2str(j) '.nii']);
        nii = make_nii(field_D_cat, vox);
        save_nii(nii,['/Volumes/LaCie/CommQSM/syn/data_for_training/field_D_cat/field_D_cat_' num2str(i) '-' num2str(j) '.nii']);
    end
end
