...
class DataConsistency(nn.Module):
    ...
    DC(chi, phi, D) = f*D*F*m*f*D*F*chi - f*D*F*m*phi
    """

    def __init__(self):
        super(DataConsistency, self).__init__()

    def forward(self, chi, phi, D, m):
        """
        f - inverse FFT
        ...
        # mask and dipole real values, have single channel, broadcasting
        D = D.permute(0, 2, 3, 4, 1)
        m = m.permute(0, 2, 3, 4, 1)

        DC = torch.ifft(D*torch.fft(m*torch.ifft(D*torch.fft(chi, 3), 3), 3), 3) - \
            torch.ifft(D*torch.fft(m*phi, 3), 3)
...
class unrolledQSM(nn.Module):
    ...
    def forward(self, x, phi, D, m):
        x = self.one_iter1(x, phi, D, m)
        x = self.one_iter2(x, phi, D, m)
        x = self.one_iter3(x, phi, D, m)
