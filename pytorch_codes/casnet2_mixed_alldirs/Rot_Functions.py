...
def get_3d_locations(d,h,w,device_):
    ...
    locations_x = torch.linspace(-w/2, w/2-1, w).view(1, 1, 1, w).to(device_).expand(1, d, h, w)
    locations_y = torch.linspace(-h/2, h/2-1, h).view(1, 1, h, 1).to(device_).expand(1, d, h, w)
    locations_z = torch.linspace(-d/2, d/2-1,d).view(1, d, 1, 1).to(device_).expand(1, d, h, w)
...
def rotate(input_tensor, rotation_matrix):
    ...
    rotated_3d_positions = torch.matmul(rotation_matrix.view(Nb, 1, 3, 3).expand(Nb, d*h*w, 3, 3), locations_3d).view(Nb, d,h,w,3)
    # shape: Nb * d * h * w, 3, 1
    ...
    """
    grid = torch.stack([normalised_locs_x, normalised_locs_y, normalised_locs_z], dim=4).view(Nb, d, h, w, 3)
