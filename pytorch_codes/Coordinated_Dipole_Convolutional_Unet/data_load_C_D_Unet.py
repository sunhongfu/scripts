...
class yangDataSet(data.Dataset):
    def __init__(self, root, list_path):
        ...
        for name in self.img_ids:
            ...
            dipole_file = self.root + ("/dipole/%s-dipole.nii" % name)
            dipole_file = self.root + ("/Coordinates/%s-Coor.nii" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "dipole": dipole_file,
