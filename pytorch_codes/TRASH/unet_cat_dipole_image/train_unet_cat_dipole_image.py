...
from dataload_unet_cat_dipole_image import *
...
def yangSaveNet(resnet, enSave=False):
    ...
    if enSave:
        torch.save(
            resnet, '/scratch/itee/uqhsun8/CommQSM/pytorch_codes/unet_cat_dipole_image/unet_cat_dipole_image.pth')
    else:
        ...
        if torch.cuda.is_available():
            ...
            for epoch in range(1, Epoches + 1):
                ...
                for i, data in enumerate(trainloader):
                    ...
                    pred = resnet(Inputs)
                    # loss
                    loss = criterion(pred[:, :, :, :, 0:48], Labels)
                    # backward
                    loss.backward()
