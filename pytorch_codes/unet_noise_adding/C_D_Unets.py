...
class Unet(nn.Module):
    def __init__(self, EncodingDepth):
        ...
        self.NoiseAddLayer = NoiseAdding(0.8)
...
################### Encoding Layers #######################
        for encodingLayer in temp:
            if encodingLayer == 1:
                ...
                self.EncodeConvs.append(EncodingBlocks(6, num_outputs))
            else:             
                num_outputs = initial_num_layers * 2 ** (encodingLayer - 1)
...
if __name__ == '__main__':
    ...
    C = torch.randn(1,3,48,48,48, dtype=torch.float)
