...
class yangDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_prjs = torch.cat(
            [label.float(), prjs.float()], 0)

        field_prjs = field_prjs.float()
        label_prjs = label_prjs.float()

        return field_prjs, label_prjs, name
...
if __name__ == '__main__':
    ...
    z_prjs_file = '/Users/uqhsun8/Documents/MATLAB/scripts/pytorch_codes/image_unet_stack_prjs_alldirs_equalsize/z_prjs_alldirs.txt'
    Batch_size = 4
    ...
    for i, Data in enumerate(trainloader):
        field_prjs, labels, names = Data
        print(i)
        if i % 1 == 0:
            print(names)
            print(field_prjs.size())
            print(labels.size())
