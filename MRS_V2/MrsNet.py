...
class MRSNet(nn.Module):
    def __init__(self, EncodingDepth=10):
        super(MRSNet, self).__init__()
        initial_num_layers = 64
        No_channels = 1
        No_out_channels = 256
        self.EncodingDepth = EncodingDepth
        self.init1 = CConv1d_BN_RELU(No_channels, No_out_channels)

        self.midLayers = []
        temp = list(range(1, EncodingDepth + 1))
        for encodingLayer in temp:
            self.midLayers.append(Basic_block(No_channels, No_out_channels))
        self.midLayers = nn.ModuleList(self.midLayers)

        self.FinalConv = CConv1d(No_channels, No_out_channels)
...
class Basic_block(nn.Module):
    def __init__(self, num_in, num_out):
        ...
        self.cconv2 = CConv1d_BN_RELU(num_in, num_out)
...
class CConv1d_BN_RELU(nn.Module):
    def __init__(self, num_in, num_out, ks=256, pad=0):
...
class CConv1d(nn.Module):
    def __init__(self, num_in, num_out, ks=256, pad=0, bs=True):
