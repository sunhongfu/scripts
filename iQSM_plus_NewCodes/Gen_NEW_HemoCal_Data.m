...
for FileNo = 1 : 4 * datasize
    ...
    %%hemo_value = 0.4 + 0.8 * rand;  %% original
    hemo_value = 0.4 + 1 * rand;

    if rand > 0.5
        %         hemo_value = -1 * (0.2 + 0.1 * rand); %% original
        hemo_value = -1 * (0.2 + 0.3 * rand); %% original
    end

    one_data = generate_one_chi_example([16, 16, 16]);
