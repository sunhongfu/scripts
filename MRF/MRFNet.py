...
class MRSNet(nn.Module):
    ...
    def forward(self, x_r, x_i, k0_r, k0_i, mask):
        ...
        X1, X2, X3 = self.FinalConv(x_r, x_i)
...
class CConv1d(nn.Module):
    def __init__(self, num_in, num_out, ks=1000, pad=0, bs=True):
    ...
    def forward(self, x_r, x_i):
        ...
        X2 = X[:, 1, :]
        X3 = X[:, 2, :]
        return torch.unsqueeze(X1, 1), torch.unsqueeze(X2, 1), torch.unsqueeze(X3, 1)
...
if __name__ == '__main__':
    ...
    print(x_r.dtype)
    X1, X2, X3 = mrsnet(x_r, x_i, x_r, x_i, x_r)

    print('output' + str(X1.size()))
    print('output' + str(X2.size()))
    print('output' + str(X3.size()))
