%classdef COMP_DictionaryObj
classdef COMP_DictionaryObj < handle
  ...
  methods
    ...
    %% Constructor
    %% ---------------------------------------------------------------------------
    function obj = COMP_DictionaryObj(input_file_path, input_file_name)
      ...
      % load lookup
        ...
        obj.atoms = fread(fid, iSets*iTot*2, 'single'); %complex
        fclose(fid);
...
%         print_summary(obj);
        ...
        obj.atoms  = reshape(obj.atoms ,2,iSets,iTot);
        
        obj.atoms = squeeze(obj.atoms(1,:,:)+1i*obj.atoms(2,:,:));
...
%         end
    ...
    %% take real part only
    %% ---------------------------------------------------------------------------
    function obj= make_real(obj)
        obj.atoms = singe(real(obj.atoms));
        obj.bIsComplex = false;
    end
