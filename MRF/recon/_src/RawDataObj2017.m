classdef RawDataObj2017 < handle
  ...
  methods
    ...
    %% Constructor
    %% ---------------------------------------------------------------------------
    function obj = RawDataObj2017(input_remove_os, input_file_path, input_file_name)
...
%         disp('------------------------------------------------------------');
    ...
    %% Coil compression
    %% ---------------------------------------------------------------------------
    function coil_compression(obj, lim)
        
%         if lim > (obj.Dim.nCh - 1)
%            disp('No rx channel compression used');
%         else
        
        disp('rx channel compression!');
        
        tmp = permute(obj.data,[3,1,2,4,5,6]);
        tmp = tmp(:,:);
        [U,~,~] = svd(tmp,'econ');
...
%         disp(size(U));
        ...
        tmp = ctranspose(ctranspose(tmp)*U);
%         disp(size(tmp));
        tmp = reshape(tmp,[lim, obj.Dim.nEc, obj.Dim.nSl, obj.Dim.nRe,obj.Dim.nLi, obj.Dim.nSe]);
        obj.data = permute(tmp, [2,3,1,4,5,6]);
...
%         end
    ...
    function obj = extractNoiseScan(obj,bExtract)
        if bExtract
         obj.noise   = obj.data(:,:,:,:,:,11:50); %first couple of lines may contain stimulated echos
         obj.data    = obj.data(:,:,:,:,:,51:end);
         obj.angles  = obj.angles (:,:,51:end);
         obj.Dim.nSe = obj.Dim.nSe -50;
   ...
   % ---------------------------------------------------------------------------
    ...
    %% ---------------------------------------------------------------------------
    %% MR Fingerprint Compression
    %% ---------------------------------------------------------------------------
    function [obj,U] = fingerprint_compression(obj)
      ...
      % disp('------------------------------------------------------------');
      % disp('                MR Fingerprint Compression');
      % disp('------------------------------------------------------------');
      %      %-----------------------------|------------------------------
 
      gbc.Dim     = obj.Dim;
      gbc.Dim.nSe = 40;
      gbc.Dim.nLi = gbc.Dim.nLi * obj.Dim.nSe/gbc.Dim.nSe;
 
      % gbc.data   = zeros(cell2mat(struct2cell(gbc.Dim)).');
      gbc.data      = zeros(gbc.Dim.nEc, gbc.Dim.nSl, gbc.Dim.nCh, gbc.Dim.nRe, gbc.Dim.nLi, gbc.Dim.nSe);
      gbc.angles    = zeros(gbc.Dim.nEc, gbc.Dim.nLi, gbc.Dim.nSe);
      gbc.locArray  = zeros(gbc.Dim.nEc, gbc.Dim.nSe)+1;
      
      U = zeros(obj.Dim.nSe, gbc.Dim.nSe);
      disp(size(U));
      for i = 1:gbc.Dim.nSe
          
          tmp = obj.cSetIn(1,1,:)+1;
          tmp(tmp>i) = 0;
          tmp(tmp<i) = 0;
          tmp(tmp>0) = 1;
          U(:,i) = tmp;
      ...
      for ec=1:obj.Dim.nEc
        for se=1:obj.Dim.nSe
          for li=1:obj.Dim.nLi
            loc = obj.cSetIn(ec,li,se)+1;
            gbc.data(ec,:,:,:,gbc.locArray(ec,loc),loc) = obj.data(ec,:,:,:,li,se);
            gbc.angles(ec,gbc.locArray(ec,loc),loc)     = obj.angles(ec,li,se);
            gbc.locArray(ec,loc)                        = gbc.locArray(ec,loc)+1;
          end
        end
      end
