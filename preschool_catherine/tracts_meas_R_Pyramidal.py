...
output_file_R_Pyramidal = "/home/hongfu/mnt/deepmri/preschool/project_preschool/R_Pyramidal.txt"
...
for root, dirs, files in os.walk(merge_folder):
    for name in files:
        if re.match(r".*r.?pyram.*\.nii", name, re.IGNORECASE):
            ...
            cmd = ("fslstats " + qsm_folder + "/*FA.nii -k " +
                   tract + " -M >> " + output_file_R_Pyramidal + " 2>&1")
            os.system(cmd)
            with open(output_file_R_Pyramidal, 'a') as f:
