...
output_file_CC_Genu = "/home/hongfu/mnt/deepmri/preschool/project_preschool/CC_Genu.txt"
...
for root, dirs, files in os.walk(merge_folder):
    for name in files:
        if re.match(r".*genu.*\.nii", name, re.IGNORECASE):
            ...
            cmd = ("fslstats " + qsm_folder + "/*FA.nii -k " +
                   tract + " -M >> " + output_file_CC_Genu + " 2>&1")
            os.system(cmd)
            with open(output_file_CC_Genu, 'a') as f:
