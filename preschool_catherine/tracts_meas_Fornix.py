...
output_file_Fornix = "/home/hongfu/mnt/deepmri/preschool/project_preschool/Fornix.txt"
...
for root, dirs, files in os.walk(merge_folder):
    for name in files:
        if re.match(r".*fornix.*\.nii", name, re.IGNORECASE):
            ...
            cmd = ("fslstats " + qsm_folder + "/*FA.nii -k " +
                   tract + " -M >> " + output_file_Fornix + " 2>&1")
            os.system(cmd)
            with open(output_file_Fornix, 'a') as f:
