...
output_file_L_Cingulum = "/home/hongfu/mnt/deepmri/preschool/project_preschool/L_Cingulum.txt"
...
for root, dirs, files in os.walk(merge_folder):
    for name in files:
        if re.match(r".*l.?cing.*\.nii", name, re.IGNORECASE):
            ...
            cmd = ("fslstats " + qsm_folder + "/*FA.nii -k " +
                   tract + " -M >> " + output_file_L_Cingulum + " 2>&1")
            os.system(cmd)
            with open(output_file_L_Cingulum, 'a') as f:
