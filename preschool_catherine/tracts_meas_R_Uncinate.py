...
output_file_R_Uncinate = "/home/hongfu/mnt/deepmri/preschool/project_preschool/R_Uncinate.txt"
...
for root, dirs, files in os.walk(merge_folder):
    for name in files:
        if re.match(r".*r.?unci.*\.nii", name, re.IGNORECASE):
            ...
            cmd = ("fslstats " + qsm_folder + "/*FA.nii -k " +
                   tract + " -M >> " + output_file_R_Uncinate + " 2>&1")
            os.system(cmd)
            with open(output_file_R_Uncinate, 'a') as f:
