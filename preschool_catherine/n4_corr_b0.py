...
for qsm_subject in qsm_list:
	for root, dirs, files in os.walk(merge_folder):
		for name in files:
			if re.match(qsm_subject + "_(\S+)_B0\.nii", name):
				b0_file = os.path.abspath(os.path.join(root, name))
				b0_file_n4 = b0_file[0:-4] + "_n4.nii"
				b0_file_n4_DC_corr = b0_file_n4[0:-4] + "_DC_corr.nii.gz"
				cmd = "N4BiasFieldCorrection -i " + b0_file + " -o " + b0_file_n4
				# print cmd
				# os.system(cmd)
				cmd = ("fslmaths " + b0_file_n4 + " -uthr 3000 -nan " + b0_file_n4_DC_corr + "\n"
						"gunzip -f " + b0_file_n4_DC_corr + "\n")
				print cmd
				os.system(cmd)
