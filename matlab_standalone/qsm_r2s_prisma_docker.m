function qsm_r2s_prisma_docker(path_mag, path_ph, json_file_name, path_out)
...
%   FILE_NAME    - name of the json file containing parameter structure including fields below
...
if ~ exist('json_file_name','var') || isempty(json_file_name)
    options = [];
else
    str = fileread(append(json_file_name, '.json')); % read json options file
    options = jsondecode(str); % convert string to object
end
...
if isfield(options,'r_mask')
    options.r_mask = str2double(options.r_mask);
else
    options.r_mask = 1;
end

if isfield(options,'fit_thr')
    options.fit_thr = str2double(options.fit_thr);
else
    options.fit_thr = 40;
end

if isfield(options,'bet_thr')
    options.bet_thr = str2double(options.bet_thr);
else
    options.bet_thr = 0.4;
end

if isfield(options,'bet_smooth')
    options.bet_smooth = str2double(options.bet_smooth);
else
...
if isfield(options,'t_svd')
    options.t_svd = str2double(options.t_svd);
else
    options.t_svd = 0.1;
end

if isfield(options,'smv_rad')
    options.smv_rad = str2double(options.smv_rad);
else
    options.smv_rad = 3;
end

if isfield(options,'tik_reg')
    options.tik_reg = str2double(options.tik_reg);
else
    options.tik_reg = 1e-3;
end

if isfield(options,'cgs_num')
    options.cgs_num = str2double(options.cgs_num);
else
    options.cgs_num = 500;
end

if isfield(options,'lbv_tol')
    options.lbv_tol = str2double(options.lbv_tol);
else
    options.lbv_tol = 0.01;
end

if isfield(options,'lbv_peel')
    options.lbv_peel = str2double(options.lbv_peel);
else
    options.lbv_peel = 2;
end

if isfield(options,'tv_reg')
    options.tv_reg = str2double(options.tv_reg);
else
    options.tv_reg = 5e-4;
end

if isfield(options,'inv_num')
    options.inv_num = str2double(options.inv_num);
else
    options.inv_num = 500;
end

if isfield(options,'interp')
    options.interp = str2double(options.interp);
else
    options.interp = 0;
end
