...
% ph_cmb = zeros(imsize);
% ph_cmb(:,:,:,1:2:end) = geme_cmb(img(:,:,:,1:2:end),vox,TE(1:2:end),mask);
% ph_cmb(:,:,:,2:2:end) = geme_cmb(img(:,:,:,2:2:end),vox,TE(2:2:end),mask);
ph_cmb = angle(img);
...
save_nii(nii,['RESHARP/no_corr_lfs_resharp_' num2str(tik_reg) '_' num2str(smv_rad) '.nii']);
...
save_nii(nii,['RESHARP/no_corr_sus_resharp_' num2str(tv_reg) '_' num2str(inv_num) '.nii']);
