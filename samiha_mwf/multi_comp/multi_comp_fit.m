...
% USE HALF THE ECHOES
% echo_times = echo_times(1:num_echoes/2);
% data_vol = data_vol(:,:,:,1:num_echoes/2);
% num_echoes = num_echoes/2;
% data_dim(1,4) = num_echoes;
...
%        t2_range = [1.5*2.4, 2000]; % for manuscript figures only
%         t2_range = [10, 2000]; % for manuscript figures only
    ...
    for slice = 1:data_slices
        ...
        for i = 1:data_height
            for j = 1:data_width
                ...
                % quick hack to analyze WM & GM voxels, remember to change
                % this back!!!
%                 if ( mask_vol(i,j,slice) == 2 || mask_vol(i,j,slice) == 3 )    
...
%         mean_roi_data = get_avgROI_data(roi_vol, data_vol, data_dim);
%         figure(2); hold on;
%         plot(this_echo_times,mean_roi_data(1:this_num_echoes),'r-','LineWidth', 4);
...
%         %--------------------
        ...
        xlim([t2_range(1) t2_range(2)])
%         xlim([1.5*2.4 2*1e3]) %keep T2* and T2 ranges the same for plot
        ylabel('Normalized Signal');
        if strcmp(relaxation_type,'T2')==1
            xlabel('T2 (ms)');
            title('T_2 Spectrum','FontSize',20);
...
%             legend('location', 'best','GRE data','mono-exponential fit','NNLS multi-exp fit','regularized NNLS multi-exp fit');
        ...
        %dlmwrite('MCMF.dat',[gm_IEW_t2_regNNLS(slice) 100*mwf_regNNLS(slice)], '-append');
        
        % plot residual
        RES = mean_roi_data(slice,:) - reg_multi_sig_recon(slice,:);
        figure;
        plot(echo_times,RES,'r');
        xlabel('time (ms)');
        ylabel('res (arb)');
        
        dlmwrite('MCMF_RES.dat',[RES], '-append');
        dlmwrite('MCMF_recon.dat',[reg_multi_sig_recon(slice,:)], '-append');
...
%                      data = mw_delf_regNNLS;
                ...
                case 3
                    file_name = 'MW_gmT2_regNNLS_map';
                    data = gm_MW_t2_regNNLS;
            end
