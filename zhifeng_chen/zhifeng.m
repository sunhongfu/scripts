load('/Users/uqhsun8/Downloads/img_buda_R4.mat')
% mag = squeeze(abs(img_buda(:,:,1,:)));
% ph = angle(squeeze((img_buda(:,:,1,:))).*conj(img_hybrid_sense));
% ph = angle(((img_buda)).*conj(reshape(img_hybrid_sense,[224 224 1 144])));

mag = squeeze(abs(img_pocs(:,:,1,:)));
ph = squeeze(angle(img_pocs(:,:,1,:)));

% mag = squeeze(abs(img_hybrid_sense));
% ph = squeeze(angle(img_hybrid_sense));
