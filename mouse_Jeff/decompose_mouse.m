...
for echo = 1:size(unph,4)
    [phase_resharp(:,:,:,echo), mask_resharp] = resharp(unph(:,:,:,echo),mask.*R,voxel_size,smv_rad,tik_reg,cgs_num);
end
...
% iLSQR method
for echo = 1:size(unph,4)
    chi_iLSQR(:,:,:,echo) = QSM_iLSQR(-phase_resharp(:,:,:,echo),mask_resharp,'H',z_prjs,'voxelsize',voxel_size,'niter',50,'TE',TE(echo)*1000,'B0',9.4);
end
nii = make_nii(chi_iLSQR,voxel_size);
save_nii(nii,'chi_iLSQR0.nii');
...
[PCS, DCS, composite_chi] = decompose_qsm_arr_v06(mag, chi_iLSQR, TE*1000, 9.4, savename, parallel_flag);
