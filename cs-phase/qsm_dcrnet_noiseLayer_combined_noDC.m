nii = load_nii('DCRNet_noiseLayer_combined_noDC_mag.nii');
mag_corr = double(nii.img) ;

nii = load_nii('DCRNet_noiseLayer_combined_noDC_ph.nii');
ph_corr = double(nii.img) ;

img = mag_corr.*exp(1j*ph_corr);

mkdir DCRNet_noiseLayer_combined_noDC
cd DCRNet_noiseLayer_combined_noDC
