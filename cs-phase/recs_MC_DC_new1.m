% METHOD 1: using exisitng coil sensitivities

% load sensitivity maps
nii = load_nii('../sensivitity_map/sen_mag_smooth.nii');
sen_mag = single(nii.img);

nii = load_nii('../sensivitity_map/offsets_smooth.nii');
sen_ph = (single(nii.img));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% METHOD 1: use existing coil sensitivities
...
offsets = repmat(exp(1j*sen_ph),[1,1,1,8,1]);
img = img./offsets;
img(isnan(img)) = 0;
ph_corr = angle(sum(img,5));
ph_corr(isnan(ph_corr)) = 0;
mag_corr = abs(sum(img,5));
mag_corr(isnan(mag_corr)) = 0;
...
!cp /Volumes/LaCie_Bottom/DL-CS-phase/recon_zf/QSM/unph_diff.nii .
