...
def loader(npy_file):
    complex_img = np.expand_dims(np.load(npy_file), axis=0)
    # normalized each complex image
    complex_img_normed = complex_img/np.max(np.abs(complex_img))
    return torch.from_numpy(np.concatenate((np.real(complex_img_normed), np.imag(complex_img_normed)), axis=0)).float()
