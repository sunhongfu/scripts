...
%%%% TIK-QSM
mkdir /home/hongfu.sun/data/phantom_LNQSM_brain
cd /home/hongfu.sun/data/phantom_LNQSM_brain
...
P = 1 * mask_brain + 30 * (1 - mask_brain);
...
for i = 1:length(Tik_weight)
	for j = 1:length(TV_weight)
		...
		chi = tikhonov_qsm(tfs_pad, mask_brain_pad, 1, mask_brain_pad, mask_brain_pad, 0, TV_weight(j), Tik_weight(i), 0, vox, P_pad, z_prjs, 5000);
		% nii = make_nii(chi(:,:,21:end-20),vox);
		nii = make_nii(chi,vox);
		save_nii(nii,['TIK_bb_TV_' num2str(TV_weight(j)) '_Tik_' num2str(Tik_weight(i)) '_P30_5000.nii']);
