...
%    .epi_recon  - new (1) or old (0) version of EPI recon   : 1
%
% Edited by -ps- February 2017:
% - added option to use newer recon code
% - fixed code when used on a single volume dataset (ie bypass spm align)
...
epi_recon  = options.epi_recon;


% 4.7T corey's EPI parameters
if (epi_recon)
    if ~exist('EPIopt','file')
        addpath(genpath('/home/peter/matlab/my_functions/EPIrecon_CAB_v1p7'));
    end;
    opt=EPIopt(1); % type 1 - no motion correction
    opt.multivol_flg=1;
    opt.upsmple=0;
    %opt.noask = 1; % automatically overwrite existing files (1), automatically create copies (2), or ask for each case (0)
    opt.save_type=1; % nii files only
    opt.motion_c=0;
    opt.rcvr_comb=0;
    opt.homod = 0;
else
...
str = which('qsm_epi47.m');
git_path = fileparts(str);
cd(git_path); 
setenv('path_qsm',path_qsm);
unix('git log --branches --abbrev-commit --graph --no-merges --tags > ${path_qsm}/git_log.txt');

cd(path_qsm);

% reconstruct complex image from fid file
if epi_recon==1
    [par,img_out] = EPIrecon_CAB(path_fid,opt);
else
