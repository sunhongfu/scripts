...
class ResNet(nn.Module):
    def __init__(self, EncodingDepth):
        ...
        self.EncodeEmbedding = []
        self.DecodeEmbedding = []
...
################### Encoding Layers #######################
        for encodingLayer in temp:
            if encodingLayer == 1:
                ...
                # Hongfu 
                self.EncodeEmbedding.append(nn.Sequential(nn.SiLU(), nn.Linear(3, num_outputs)))
        ...
        # Hongfu
        self.EncodeEmbedding = nn.ModuleList(self.EncodeEmbedding)
...
################### Decoding Layers #######################
        for decodingLayer in temp:
            if decodingLayer == EncodingDepth:
                ...
                # Hongfu 
                self.DecodeEmbedding.append(nn.Sequential(nn.SiLU(), nn.Linear(3, num_inputs)))
                ...
                # Hongfu 
                self.DecodeEmbedding.append(nn.Sequential(nn.SiLU(), nn.Linear(3, num_inputs // 2)))

        self.DecodeConvs = nn.ModuleList(self.DecodeConvs)
        # Hongfu
        self.DecodeEmbedding = nn.ModuleList(self.DecodeEmbedding)
...
################## End Section 2 ##########################
    ...
    def forward(self, x, z_prjs):
        ...
        for encodingLayer in temp:
            ...
            # Hongfu
            emb_layer =  self.EncodeEmbedding[encodingLayer - 1]
            x = x + emb_layer(z_prjs)[:, :, None, None, None].repeat(1, 1, x.shape[-3], x.shape[-2], x.shape[-1])
        ...
        for decodingLayer in temp:
            ...
            # Hongfu
            emb_layer =  self.DecodeEmbedding[decodingLayer - 1]
            x = x + emb_layer(z_prjs)[:, :, None, None, None].repeat(1, 1, x.shape[-3], x.shape[-2], x.shape[-1])
