...
%   Description
%
%   B = SHARPEDGES(A,ROI,reducedROI) extends harmonic field A, defined over reducedROI,
%   into the (extended) ROI, returning structure B which contains the following fields:
%
%        .extendedBackgroundField
%                 .extendedROI 
%        .numPointsExtendedROI
%      .reducedBackgroundField
%            .IPtoEdgeDistance
%    .associatedIPtoEPDistance
%           .IPtoEPDistanceXYZ
%               .associatedIPs
%                         .ROI 
%                  .reducedROI 
...
elseif ~license('test', 'Distrib_computing_toolbox') && Options.numCPU > 1 ;
    disp('Distributed computing toolbox unavailable. Check licenses.')
    Options.numCPU = DEFAULT_NUMCPU ;
end
