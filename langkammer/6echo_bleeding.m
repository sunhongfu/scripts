nii1 = load_untouch_nii('/Volumes/LaCie_Bottom/collaborators/Christian_Langkammer/55_gre_with_bleedings/20200907_112645gre6EinPhase3D2mms020a1001.nii');
mag = single(nii1.img);

nii2 = load_untouch_nii('/Volumes/LaCie_Bottom/collaborators/Christian_Langkammer/55_gre_with_bleedings/20200907_112645gre6EinPhase3D2mms021a1001.nii');
pha = single(nii2.img);
...
mkdir('55_gre_with_bleedings_recon')
cd('55_gre_with_bleedings_recon')
...
ph_corr(:,:,:,1:2:end) = geme_cmb(mag(:,:,:,1:2:end).*exp(1j*pha(:,:,:,1:2:end)),vox,TE(1:2:end),mask);
ph_corr(:,:,:,2:2:end) = geme_cmb(mag(:,:,:,2:2:end).*exp(1j*pha(:,:,:,2:2:end)),vox,TE(2:2:end),mask);
mag_corr = mag;
